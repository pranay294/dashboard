...
(function($) {
		...
		colPosition = 0; // for printing colors on simple bar graph
 		...
 		if(arr.legend){
			$(el).append("<div id='legendHolder"+unique+"'></div>");
	 		$('#legendHolder'+unique).css({ 'width': legendWidth, 'float': 'left', 'text-align' : 'left', 'position' : 'relative', 'top' : '-45px', 'left' : '5px' });
	...
	$.fn.jqBarGraph.defaults = {	
		...
		position: 'top', // or 'top' doesn't work for multi type
		prefix: '',
		postfix: '',
		animate: true,
		speed: 1.5,
